...
- `comparisons.html` → the side-by-side comparison sheets
...
comparisons.html        comparison tables
assets/
  ...
  js/render-comparisons.js  builds the comparison tabs + tables
products/
  marrakesh.js          ONE product = ONE file (markdown is the source of truth)
comparisons/
  jacket-comparison.js  ONE comparison = ONE file
```

On screens narrower than 1200px the comparison tables switch to a picker: the
chip row above the table chooses which products are columns. It opens with as
many as comfortably fit — two on a phone, five or six on a small laptop — the
row labels stay pinned while you scroll sideways, and **Show all** brings the
whole sheet back. Wider screens get the full sheet as before.
